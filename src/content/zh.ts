...
export const zh: SiteCopy = {
  ...
  nav: {
    ...
    tools: {
      ...
      roadmap: { label: '路线图', blurb: '做完了什么，接下来做什么' },
  ...
  thesis: {
    ...
    pillars: [
      {
        title: '一切始于一个问题',
        body: '你后悔过的每一笔交易，都始于「如果当初」。$IF 就是那个念头被铸成了币——它关于那种感觉，而不是对接下来会发生什么的预测。',
      },
      {
      ...
      {
        title: '已经销毁，永不回来',
        body: '{burned} 枚 $IF（占供应量 {burnedPercent}）躺在一个没有人拿得到私钥的地址里。不是计划，也不是时间表：是你现在就能读到的余额。',
      },
      {
        title: '别相信这个页面',
        body: '合约、流动性池、销毁记录和持有人数量全部公开，本页每一个数字都链接到它的来源。自己核实，然后再决定。',
      },
    ],
  },
  ...
  faq: {
    ...
    items: [
      ...
      {
        q: '有交易税吗？',
        a: '$IF 合约不会在你的交易之上额外收取任何费用——它里面根本没有任何费用逻辑。你在主池看到的 1% 是 Uniswap 自己的费率档位，由各个流动性头寸赚取。其中最大的一个是已永久锁定的发行时流动性头寸，它那一份被创建该代币的发行平台归集：$IF 的那一边被销毁，ETH 的那一边不会。这当中没有任何一部分流向这个社区。完整的流向见白皮书。',
      },
      {
        q: '销毁了多少？',
  ...
  pages: {
    ...
    stats: {
      ...
      chartOptions: '选项',
      candlesAsOf: 'K 线截至 {time} —— 暂时无法获取实时数据',
      concentrationNote: '前十名包含销毁地址和流动性池，两者都不是个人持有者。',
      ...
      viewTx: '查看',
      loading: '正在读取链上数据…',
      feedUnavailable: '无法读取最新成交。正在重试。',
      chartUnavailable: '目前无法获取价格历史。',
    ...
    docs: {
      title: '$IF 白皮书',
      description: '$IF 是什么、不是什么、怎么买，以及如何自己核对这里的每一个数字。',
      eyebrow: '白皮书',
      heading: { lead: '关于 $IF 的一切，', accent: '白纸黑字' },
      intro:
        '这个币是什么、不是什么、怎么买，以及逐条给出的命令 —— 让你自己核对这里的每一个数字，不必相信我们的任何一句话。',
      contractLabel: '合约地址',
      contractNote: 'Robinhood Chain 上只有这一个地址。其他的都不是这个币。',
      groups: {
        coin: '这个币',
        proof: '证据',
        underTheHood: '底层机制',
    ...
    roadmap: {
      ...
      tracks: {
        proof: {
          label: '验证',
          objective: '本站的每一个数字，都能由一个并不信任我们的人，自己从链上复原出来。',
        },
        tools: {
          label: '工具',
          objective: '值得点开的东西。没有一个会要求你连接钱包或签名。',
        },
        words: {
          label: '文字',
          objective:
            '用你思考时使用的语言来阅读 —— 关于如何保护自己的页面，排在关于我们的页面前面。',
        },
        craft: {
          label: '打磨',
          objective: '它必须在旧手机上、在糟糕的网络下、第一次就能用。',
        },
      },
      needsLabel: '等待',
      signalLabel: '完成条件',
      ...
      terms: {
        title: '这份路线图不会做的事',
        items: [
          '给还没发生的事情定日期。已上线的条目带日期，因为那些已经发生了；给其余的事情定日期，是加密世界里最容易被打破的承诺。',
          '提到价格、市值或目标价。本页面上的任何内容，都不构成购买任何东西的理由。',
          '在一项合作、上所或集成真正存在、并且能被打开之前，就把它列出来。',
          '做任何要求你连接钱包或签名的东西。如果这一点将来有变，它会先作为这个页面上的一行字出现，然后才会作为一项功能出现。',
        ],
      },
    },
    holdings: {
      ...
      download: '下载卡片',
      postOnX: '发到 X',
      verify: '在 Blockscout 上核对',
      shareOfSupply: '占全部 $IF 的 {share}',
      ...
      verdicts: {
        ...
        burn: '没有人持有这个地址的私钥。这里的每一枚都永远消失了。',
        pool: '这是市场本身，不是持有者。每一笔交易都经过这里。',
      },
      errors: {
        empty: '请先粘贴一个地址。',
