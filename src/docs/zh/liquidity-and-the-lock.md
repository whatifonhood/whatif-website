...
下面每一个数字都是在 2 September 2026、约在区块 `52,582,784` 处从链上读取的。池子的余额会变。锁不会。
...
有意思的是第二行。Uniswap V3 上的 IF/USDG 池是由发行平台之外的人在 6 August 创建的，它现在承载了 $IF 全部成交量的大约三分之一，对手边是 Robinhood Chain 自己的美元稳定币。其中没有任何一部分被锁定。这是字面意义上的社区流动性：一个人选择把它放进去，也可以选择把它取出来。
