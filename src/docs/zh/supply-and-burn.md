...
当前的销毁余额在[数据页](/zh/stats/)上实时读取，每一次单独的销毁都被列出，并附有产生它的那笔交易的链接。在 2 September 2026 采集的一次快照中，它是 93,449,236.90 $IF，占供应量的 9.3449%。构成这个总数的 425 笔转账中，有两笔来自普通钱包，两笔加起来是 2.84 $IF。另外 423 笔来自同一个手续费合约，也就是下面描述的那个机制。
...
由创建该代币的发行平台运行的一个 keeper 程序，会把累积的手续费从那个头寸里扫出来。至今已有 423 次扫出，第一次在 11 July 2026，最近一次在 1 September 2026。在最初的十次里，发行平台销毁了它所收取的 $IF 的 80%，其余留下。在 12 July 2026 之后的全部 413 次里，它把 $IF 那一边全额销毁。[销毁是怎么运作的](/zh/docs/how-the-burn-works/)列出了这些地址、分成，以及手续费的 ETH 那一边去了哪里。
...
成交量同时决定了它的规模，而成交量已经比发行那一周下降了很多。在 2 September 2026 那次快照前后，供应量大约每天缩减 0.014%。那是一个真实的机制，产出一个很小的数字，它不构成对价格抱有预期的理由。
